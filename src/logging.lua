...
local _MODULE = {_VERSION = '0.1.0', LOGGERS = LOGGERS}

local function createLogger(name, lvl)
    local log_levels = {TRACE = 10, DEBUG = 20, INFO = 30, WARN = 40, ERROR = 50, PRINT = 1000}
    return {
        name = name,
        ...
        log = function(self, level, ...)
            ...
            local message = {
                ...
                formatted = function(self, dump)
                    ...
                    if dump then
                        return string.format('%s [%s] - %s :: %s', generateDateTime(self.time), self.name, self.level,
                                             self.text)
                    end
...
function generateDateTime(start)
    ...
    local dateTime = dateTimeTable.year .. '-' .. addZeroForLessThan10(dateTimeTable.month) .. '-' ..
                         addZeroForLessThan10(dateTimeTable.day) .. '-' .. addZeroForLessThan10(dateTimeTable.hour) ..
